...
class EnvironmentStatusDto(BaseModel):
    ...
    """
    applications: conlist(ApplicationStatusDto) = Field(...)
    containers: conlist(ApplicationStatusDto) = Field(...)
    databases: conlist(DatabaseStatusDto) = Field(...)
    helms: conlist(ApplicationStatusDto) = Field(...)
    id: StrictStr = Field(...)
    jobs: conlist(ApplicationStatusDto) = Field(...)
    project_id: StrictStr = Field(...)
    state: ServiceStateDto = Field(...)
    additional_properties: Dict[str, Any] = {}
    __properties = ["applications", "containers", "databases", "helms", "id", "jobs", "project_id", "state"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> EnvironmentStatusDto:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> EnvironmentStatusDto:
        """Create an instance of EnvironmentStatusDto from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return EnvironmentStatusDto.parse_obj(obj)

        _obj = EnvironmentStatusDto.parse_obj({
